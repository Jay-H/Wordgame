...
func _process(delta):
	timer_updater()
	if bonus_letter_pressed == true:
		current_bonus_node.modulate = Color.LIGHT_GRAY
		current_bonus_node.add_theme_color_override("font_color", Color.LIGHT_GRAY)
		tween2.stop()
...
func bonus_letter_spawner():
	...
	# Resume the bonus color change tween after the process function has stopped it because the letter went gray
	tween2.play()
